document.addEventListener('DOMContentLoaded', () => {
    ...
    // BORRA TODA LA FUNCIÓN loadCommunityPage QUE TENGAS Y REEMPLÁZALA CON ESTA VERSIÓN FINAL
    ...
    async function loadCommunityPage() {
        const elements = {
            ...
            createGroupBtn: document.getElementById('create-group-btn'), // Estos elementos siguen estando en el HTML pero no se usan
            ...
            generalChatBadge: document.getElementById('general-chat-badge'),
            friendsBadge: document.getElementById('friends-badge'),
            requestsBadge: document.getElementById('requests-badge'),
            typingIndicator: document.getElementById('typing-indicator') // Nuevo
        };

        let allUsers = {}, userStatuses = {}, activeChat = { type: 'general', id: 'general' }, currentChatListener = null;
        let unreadCounts = { general: 0, private: {}, requests: 0 }; // NUEVO: para almacenar contadores de no leídos
        let typingStatusListener = null; // NUEVO: para el listener de estado de escritura
        let typingTimeout = null; // NUEVO: para gestionar el "dejó de escribir"

        // --- FUNCIONES DE NOTIFICACIONES ---
        const updateNotificationBadge = (element, count) => {
            if (count > 0) {
        ...
        const updateTitleNotification = () => {
            const totalUnread = unreadCounts.general + unreadCounts.requests + Object.values(unreadCounts.private).reduce((a, b) => a + b, 0);
            if (totalUnread > 0) {
                document.title = `(${totalUnread}) StudyVerse - Comunidad`;
            } else {
                document.title = `StudyVerse - Comunidad`;
            }
        };
        ...
        const renderChatNav = () => {
            ...
            <li class="chat-nav-item ${activeChat.id === 'general' ? 'active' : ''}" data-type="general" data-id="general">
                <div class="avatar-emoji">💬</div><span>General</span>
                <span class="notification-badge ${unreadCounts.general === 0 ? 'hidden' : ''}" id="general-chat-badge">${unreadCounts.general}</span>
            </li>
            <li class="chat-nav-item" data-type="friends">
                <div class="avatar-emoji">👥</div><span>Amigos</span>
                <span class="notification-badge ${Object.values(unreadCounts.private).reduce((a, b) => a + b, 0) === 0 ? 'hidden' : ''}" id="friends-badge">${Object.values(unreadCounts.private).reduce((a, b) => a + b, 0)}</span>
            </li>
            <li class="chat-nav-item" data-type="requests">
                <div class="avatar-emoji">📥</div><span>Solicitudes</span>
                <span class="notification-badge ${unreadCounts.requests === 0 ? 'hidden' : ''}" id="requests-badge">${unreadCounts.requests}</span>
            </li>
            ...
            if (currentUserData.friends) {
                Object.keys(currentUserData.friends).forEach(friendId => {
                    ...
                    if (friend) {
                        ...
                        <span class="notification-badge ${unreadCounts.private[friendId] === 0 || !unreadCounts.private[friendId] ? 'hidden' : ''}">${unreadCounts.private[friendId] || ''}</span>`;
        ...
        const switchChat = (type, id) => {
            ...
            // Detener listener de escritura anterior si existe
            if (typingStatusListener) typingStatusListener.off();
            elements.typingIndicator.classList.add('hidden'); // Ocultar indicador al cambiar de chat
            ...
            let chatRef;
            let typingChatId = null; // Para el estado de escritura

            if (type === 'general') {
                elements.chatTitle.textContent = '#general';
                elements.chatMessageInput.placeholder = 'Escribe en #general...';
                chatRef = db.ref('chats/general');
                renderSidebar('general');
                // Marcar mensajes como leídos en el chat general
                db.ref(`unreadCounts/${currentUserData.uid}/general`).set(0);
            } else if (type === 'private' && allUsers[id]) {
                const friendData = allUsers[id];
                elements.chatTitle.textContent = friendData.username;
                elements.chatMessageInput.placeholder = `Escribe a ${friendData.username}...`;
                const chatId = currentUserData.uid < id ? `${currentUserData.uid}_${id}` : `${id}_${currentUserData.uid}`;
                chatRef = db.ref(`privateChats/${chatId}`);
                typingChatId = chatId; // Establecer chatId para el estado de escritura
                renderSidebar('friends');
                // Marcar mensajes como leídos en el chat privado
                db.ref(`unreadCounts/${currentUserData.uid}/private/${id}`).set(0);
            } else {
                elements.chatView.classList.add('hidden');
                elements.noChatSelectedView.classList.remove('hidden');
                return;
            }

            // Configurar listener de escritura para el chat actual
            if (typingChatId) {
                typingStatusListener = db.ref(`typingStatus/${typingChatId}`).on('value', snapshot => {
                    const status = snapshot.val();
                    if (status && status.isTyping && status.userId !== currentUserData.uid) {
            ...
            if (chatRef) {
                currentChatListener = chatRef.limitToLast(100);
                currentChatListener.on('child_added', snapshot => {
                    renderMessage(snapshot);
                    // Incrementar contador de no leídos para el otro usuario si no está en este chat
                    const msg = snapshot.val();
                    if (msg.userId === currentUserData.uid) return; // No contar mis propios mensajes
                    if (activeChat.type === 'general' && activeChat.id !== 'general') { // Estoy en otro chat
                        db.ref(`unreadCounts/${currentUserData.uid}/general`).transaction(current => (current || 0) + 1);
                    } else if (activeChat.type === 'private' && activeChat.id !== id) {
                        db.ref(`unreadCounts/${currentUserData.uid}/private/${id}`).transaction(current => (current || 0) + 1);
                    }
                });
            }
            renderChatNav();
        ...
        const deleteFriend = (friendUid) => {
            ...
            db.ref().update(updates).then(() => {
                if (activeChat.type === 'private' && activeChat.id === friendUid) {
        ...
        // NUEVO: Listener para el input del mensaje (para el estado de escritura)
        elements.chatMessageInput.addEventListener('input', () => {
            if (activeChat.type !== 'private') return; // Solo para chats privados

            const chatId = currentUserData.uid < activeChat.id ? `${currentUserData.uid}_${activeChat.id}` : `${activeChat.id}_${currentUserData.uid}`;
            db.ref(`typingStatus/${chatId}`).set({ isTyping: true, userId: currentUserData.uid });

            clearTimeout(typingTimeout);
            typingTimeout = setTimeout(() => {
                db.ref(`typingStatus/${chatId}`).set({ isTyping: false, userId: currentUserData.uid });
            }, 3000); // Se considera que dejó de escribir después de 3 segundos sin actividad
        });
        ...
        elements.chatForm.addEventListener('submit', (e) => {
            ...
            if (!text) return;
            const message = { userId: currentUserData.uid, text: text, timestamp: firebase.database.ServerValue.TIMESTAMP };
            let messageRef;
            let recipientId = null; // Para incrementar el contador de no leídos al otro usuario

            if (activeChat.type === 'general') {
                messageRef = db.ref('chats/general').push();
                // Incrementar contador de no leídos para todos los demás usuarios (no en este ejemplo sencillo)
            } else if (activeChat.type === 'private') {
                const chatId = currentUserData.uid < activeChat.id ? `${currentUserData.uid}_${activeChat.id}` : `${activeChat.id}_${currentUserData.uid}`;
                messageRef = db.ref(`privateChats/${chatId}`).push();
                recipientId = activeChat.id; // El otro usuario en el chat privado
            }

            if (messageRef) {
                messageRef.set(message).then(() => {
                    elements.chatMessageInput.value = '';
                    // Desactivar el estado de escritura después de enviar el mensaje
                    if (activeChat.type === 'private') {
                        const chatId = currentUserData.uid < activeChat.id ? `${currentUserData.uid}_${activeChat.id}` : `${activeChat.id}_${currentUserData.uid}`;
                        db.ref(`typingStatus/${chatId}`).set({ isTyping: false, userId: currentUserData.uid });
                        clearTimeout(typingTimeout);
                    }
                    // NUEVO: Incrementar el contador de no leídos del RECEPTOR
                    if (recipientId) {
                        db.ref(`unreadCounts/${recipientId}/private/${currentUserData.uid}`).transaction(current => (current || 0) + 1);
                    } else if (activeChat.type === 'general') {
                        // Lógica para el chat general (más compleja, se haría con Cloud Functions para notificar a todos)
                        // Por ahora, solo actualizamos el título si otros usuarios reciben el mensaje
                    }
                });
            }
        });

        // --- CARGA INICIAL Y LISTENERS DE ACTUALIZACIÓN EN TIEMPO REAL ---
        ...
        userStatuses = statusSnapshot.val() || {};
        ...
        // NUEVO: Listener para los contadores de no leídos del usuario actual
        db.ref(`unreadCounts/${currentUserData.uid}`).on('value', snapshot => {
            const counts = snapshot.val() || {};
            unreadCounts.general = counts.general || 0;
            unreadCounts.requests = (currentUserData.friendRequests?.received ? Object.keys(currentUserData.friendRequests.received).length : 0); // Las solicitudes no son en base de datos
            unreadCounts.private = counts.private || {};
            renderChatNav(); // Se redibuja la navegación para actualizar los badges
        });

        db.ref(`users/${currentUserData.uid}`).on('value', async (userSnapshot) => {
            if (!userSnapshot.exists()) return;
            const previousFriendsCount = Object.keys(currentUserData.friends || {}).length;
            const previousRequestsCount = Object.keys(currentUserData.friendRequests?.received || {}).length;

            currentUserData = { uid: currentUserData.uid, ...userSnapshot.val() };

            // Actualizar contador de solicitudes
            unreadCounts.requests = (currentUserData.friendRequests?.received ? Object.keys(currentUserData.friendRequests.received).length : 0);
            if (unreadCounts.requests !== previousRequestsCount) {
                renderChatNav(); // Forzar el renderizado de la nav si las solicitudes cambian
            }

            renderChatNav();
            const activeSidebar = document.querySelector('#sidebar-content > ul:not(.hidden)');
            if (activeSidebar) {
                const viewType = activeSidebar.id.replace('-list', '');
                if (viewType !== 'groups') {
                    renderSidebar(viewType);
                } else {
                    renderSidebar('general');
                }
            }
        ...
        // Asegurarse de que el contador de solicitudes se actualiza en el inicio
        unreadCounts.requests = (currentUserData.friendRequests?.received ? Object.keys(currentUserData.friendRequests.received).length : 0);

        // Finalmente, iniciar el chat general
        switchChat('general', 'general');
    }
